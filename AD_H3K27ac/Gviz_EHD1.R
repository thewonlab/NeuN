...
genelist<-c("EHD1")
chromosome=unique(neunneg_hyper[neunneg_hyper[,8]==genelist,1])
peak_start=unique(neunneg_hyper[neunneg_hyper[,8]==genelist,9])
peak_end=unique(neunneg_hyper[neunneg_hyper[,8]==genelist,10])
...
window<-GRanges(seqnames=chromosome, 
         IRanges(min(start(tss_neg),start(Enh_neg))-50000, 
                max(end(tss_neg),end(Enh_neg))+10000))
##Gene model
...
plotTracks(c(itrack,gtrack,htrack1,ht,htrack4,tad_track1,tad_track2),from=start(window),to=end(window),
