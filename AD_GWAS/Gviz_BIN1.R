...
genelist<-c("BIN1") 
chromosome <- paste0('chr',geneAnno1[geneAnno1$hgnc_symbol %in% genelist,3])

snp <- read.csv("/proj/hyejunglab/disorder/Alzheimer/commonvar/Jansen_TableS8.csv",header=T)
snp <- snp[snp$Credible.Causal=='Yes','SNP']
load("/proj/hyejunglab/crossdisorder/GWAS_Data/Alzheimers.CTG.2018/AD_isnpranges.rda") #all SNPs for AD 
SNP <- as.data.frame(isnpranges)
SNP$seqnames <- paste0('chr',SNP$seqnames)
crSNP <- SNP[SNP$rsid=='rs6733839',c(1:3,9)]
SNP <- SNP[SNP$P <=0.05,]
snp <- SNP[SNP$rsid %in% snp,c(1:3,9)] #credible SNPs
snp$P <- -log10(snp$P)
SNP <- SNP[c(1:3,9)]
SNP$P <- -log10(SNP$P)
crSNP$P <- -log10(crSNP$P)
...
window<-GRanges(seqnames=chromosome, 
         IRanges(min(start(tss_neg),start(Enh_neg))-100000, 
                max(end(tss_neg),end(Enh_neg))+100000))
...
##track
tadtrack<-function(tadranges,sample_id,col_id,chromosome){
...
tad_track1<-tadtrack(tad_pos_ran,"NeuNpos TAD","#339900",chromosome)
tad_track2<-tadtrack(tad_neg_ran,"NeuNneg TAD","#993366",chromosome)

peak_track3<-tadtrack(neuronranges,"NeuNpos peak","#339900",chromosome)
peak_track4<-tadtrack(gliaranges,"NeuNneg peak","#993366",chromosome)
...
peak_track5<-DataTrack(range=SNP,genome = "hg19",name="all_snp",
                      type="p",chromosome=chromosome)
peak_track6<-DataTrack(range=snp,genome = "hg19",name="credible_snp",
                      type="p",chromosome=chromosome,col="orange")
peak_track7<-DataTrack(range=crSNP,genome = "hg19",name="credible_snp",
                      type="p",chromosome=chromosome,col="red")
ot <- OverlayTrack(trackList=list(peak_track5, peak_track6, peak_track7))
...
pdf(paste0("Loops_",genelist,"_2.pdf"),width=7,height=7)
plotTracks(c(itrack,gtrack,htrack1,ot,peak_track1,peak_track3,peak_track2,peak_track4,htrack4,tad_track1,tad_track2),from=start(window),to=end(window),
    transcriptAnnotation="symbol",add53=TRUE,showBandID=TRUE,cex.bands=0.5,stackHeight=0.5,
