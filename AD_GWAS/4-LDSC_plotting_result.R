...
library(reshape)
library(ggplot2)
setwd("/proj/hyejunglab/crossdisorder/LDSC/partherit/results/phase3/NeuN/")
...
save(neunpval, neunenrich, neunherit, neunse, conditions,
     file="NeuN_H3K27ac_enrichment_LDSC.rda")

load("NeuN_H3K27ac_enrichment_LDSC.rda")
...
cellname<-c("adhd2","asd2","bp2019","scz3","mdd.2019","ad.jansen","pd2019","ms","als")

pdf(file=paste0("NeuN_enrichment_disorder.pdf"), width=8, height=5)
...
barpl<-ggplot(neunpvalmelt, aes(x=factor(conditions,levels=cellname),y=-log10(FDR),fill=celltype))
barpl + geom_bar(stat="identity",position="dodge") + theme_bw() + ggtitle("FDR Enrichment") + scale_fill_brewer(palette="Paired") +
...
barpl<-ggplot(neunenrichmelt, aes(x=factor(conditions,levels=cellname),y=value,fill=celltype))
barpl + geom_bar(stat="identity",position="dodge",colour="white") + theme_bw() + ggtitle("Enrichment_score") + ylab("Heritability Enrichment") + scale_fill_brewer(palette="Paired") +
