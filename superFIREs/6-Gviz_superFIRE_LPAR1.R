...
itrack <- IdeogramTrack(genome="hg19", chromosome ="chr9")
gtrack <- GenomeAxisTrack(genome="hg19", chromosome ="chr9",
...
pos_atrack <- AnnotationTrack(lwd=1,stacking="dense",pos_FIRE, genome="hg19", chromosome="chr9", name="NeuNpos", rotate.title=360)
neg_atrack <- AnnotationTrack(lwd=1,stacking="dense",neg_FIRE, genome="hg19", chromosome="chr9", name="NeuNneg", rotate.title=360)
biomTrack <- BiomartGeneRegionTrack(genome="hg19",symbol="LPAR1",
name="ENSEMBL", biomart=bm)
dTrack <- DataTrack(FIRE_pvalue,genome="hg19", chromosome="chr9", type=c("s"), legend=TRUE,groups=c("NeuN+_-log10(pvalue)", "NeuN-_-log10(pvalue)"),col=c("brown1","darkcyan"),name="-log10(pvalue)")

pdf("superFIRE_LPAR1.pdf")
plotTracks(list(itrack,gtrack,biomTrack,pos_atrack,neg_atrack,dTrack),from=113000000,to=114000000,
