...
Track_new<-function(enh_range,pro_range,hic_name,chr_name){
    ...
    ##To visualize Hi-C in GViz, need to create an alignments track
    ##Within the alignments track you have to indicate through the CIGAR that the
    ##reads are split aligned in order to draw a sashimi plot
    ##For CIGAR definitions, see SAM https://samtools.github.io/hts-specs/SAMv1.pdf
    ##Overlap first with the region
...
chromosome="chr5"  #Define the chromosome number
...
GeneModel<-BiomartGeneRegionTrack(genome="hg19", biomart=mart, chromosome=chromosome,
            start=78500000, end=80000000, showId=T, geneSymbols=T,
...
plotTracks(c(itrack,gtrack,htrack1,peak_track1,peak_track3,peak_track2,peak_track4,htrack3,tad_track1,tad_track2),collapseTranscripts="meta",shape="arrow",from=78500000,to=80000000,
    ...
    just.group="right",cex.main=0.8);  # collapseTranscripts="meta" htrack4,
dev.off()
